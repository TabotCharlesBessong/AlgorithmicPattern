...
const strongPasswordChecker = (s) => {
  ...
  let numSubstringBreaks = substringLengths.filter(len => len >= 3).reduce((sum,len) => sum + Math.floor(len / 3),0)

  let numInsertions = Math.max(0, 6 - s.length)
  console.log(numDeletions,numInsertions,numMissingTypes,numSubstringBreaks)

  return numDeletions + Math.max(numMissingTypes, numInsertions,numSubstringBreaks)
}
